...
const researchBoxIngredients = document.getElementById(
  "researchBox--ingredients"
);
...
const researchBoxIngredientsExpanded = document.getElementById(
  "researchBox--ingredients__expanded"
);
...
const researchInputIngredients = document.getElementById(
  "research__input__ingredients"
);
const researchBoxDevicesExpanded = document.getElementById(
  "researchBox--devices__expanded"
);
const devicesBox = document.getElementById("devicesBox");
const closeDevices = document.getElementById("closeDevices");
const researchInputDevices = document.getElementById(
  "research__input__devices"
);
const researchBoxUtensilsExpanded = document.getElementById(
  "researchBox--utensils__expanded"
);
const utensilsBox = document.getElementById("utensilsBox");
const closeUtensils = document.getElementById("closeUtensils");
const researchInputUtensils = document.getElementById(
  "research__input__utensils"
);
const tagsBox = document.getElementById("tagsBox");
const searchBar = document.getElementById("searchBar");

//Events
researchBoxIngredients.addEventListener(
  "click",
  openResearchBoxIngredientsExpanded
);
...
function makeRecipesCards() {
  for (let a = 0; a < recipesToDisplay.length; a++) {
    ...
    recipeCardDescriptionHeader.classList.add(
      "recipeCard__description__header"
    );
    recipeCardDescriptionHeaderTitle.classList.add(
      "recipeCard__description__header__title"
    );
    recipeCardDescriptionHeaderTimeBox.classList.add(
      "recipeCard__description__header__timeBox"
    );
    recipeCardDescriptionHeaderTimeBoxTime.classList.add(
      "recipeCard__description__header__timeBox__time"
    );
    recipeCardDescriptionIngredients.classList.add(
      "recipeCard__description__ingredients"
    );
    recipeCardDescriptionRecipe.classList.add(
      "recipeCard__description__recipe"
    );

    //Content
    recipeCardDescriptionHeaderTitle.innerText = recipesToDisplay[a].name;
    recipeCardDescriptionHeaderTimeBox.innerHTML =
      '<svg aria-hidden="true" focusable="false" data-prefix="far" data-icon="clock" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="currentColor" d="M256 8C119 8 8 119 8 256s111 248 248 248 248-111 248-248S393 8 256 8zm0 448c-110.5 0-200-89.5-200-200S145.5 56 256 56s200 89.5 200 200-89.5 200-200 200zm61.8-104.4l-84.9-61.7c-3.1-2.3-4.9-5.9-4.9-9.7V116c0-6.6 5.4-12 12-12h32c6.6 0 12 5.4 12 12v141.7l66.8 48.6c5.4 3.9 6.5 11.4 2.6 16.8L334.6 349c-3.9 5.3-11.4 6.5-16.8 2.6z"></path></svg>';
    recipeCardDescriptionHeaderTimeBoxTime.innerText =
      recipesToDisplay[a].time + " min";
    ...
    recipeCardDescriptionHeaderTimeBox.appendChild(
      recipeCardDescriptionHeaderTimeBoxTime
    );
    ...
    for (let i = 0; i < recipesToDisplay[a].ingredients.length; i++) {
      //DOM Elements
      const recipeCardDescriptionIngredientsIngredient =
        document.createElement("div");
      const recipeCardDescriptionIngredientsIngredientName =
        document.createElement("p");
      const recipeCardDescriptionIngredientsIngredientQuantity =
        document.createElement("p");

      //Classes
      recipeCardDescriptionIngredientsIngredient.classList.add(
        "recipe__card__description__ingredients__ingredient"
      );
      recipeCardDescriptionIngredientsIngredientName.classList.add(
        "recipe__card__description__ingredients__ingredient__name"
      );
      recipeCardDescriptionIngredientsIngredientQuantity.classList.add(
        "recipe__card__description__ingredients__ingredient__quantity"
      );

      //Content

      if (recipesToDisplay[a].ingredients[i].quantity != undefined) {
        recipeCardDescriptionIngredientsIngredientName.innerText =
          recipesToDisplay[a].ingredients[i].ingredient + " :";
        if (recipesToDisplay[a].ingredients[i].unit != undefined) {
          recipeCardDescriptionIngredientsIngredientQuantity.innerText =
            recipesToDisplay[a].ingredients[i].quantity +
            " " +
            recipesToDisplay[a].ingredients[i].unit;
        } else {
          recipeCardDescriptionIngredientsIngredientQuantity.innerText =
            recipesToDisplay[a].ingredients[i].quantity;
        }
      ...
      //Nodes
      recipeCardDescriptionIngredients.appendChild(
        recipeCardDescriptionIngredientsIngredient
      );
      recipeCardDescriptionIngredientsIngredient.appendChild(
        recipeCardDescriptionIngredientsIngredientName
      );
      recipeCardDescriptionIngredientsIngredient.appendChild(
        recipeCardDescriptionIngredientsIngredientQuantity
      );
    }
  }
}
  ...
  for (let i = 0; i < recipes.length; i++) {
    ...
    for (let y = 0; y < recipes[i].ingredients.length; y++) {
      let ingredientToLowerCase =
        recipes[i].ingredients[y].ingredient.toLowerCase();
      if (ingredientToLowerCase.includes(searchToLowerCase)) {
        ingredientExist = true;
      }
    }
    if (
      nameToLowerCase.includes(searchToLowerCase) ||
      descriptionToLowerCase.includes(searchToLowerCase) ||
      ingredientExist
    ) {
      let allIngredientsTagAreGood = true;
      for (let j = 0; j < researchConstraints.ingredients.length; j++) {
        let ingredientTagToLowerCase =
          researchConstraints.ingredients[j].toLowerCase();
        let ingredientTagExist = false;
        for (let a = 0; a < recipes[i].ingredients.length; a++) {
          let ingredientToLowerCase =
            recipes[i].ingredients[a].ingredient.toLowerCase();
      ...
      if (allIngredientsTagAreGood) {
        ...
        for (let j = 0; j < researchConstraints.devices.length; j++) {
          let deviceTagToLowerCase =
            researchConstraints.devices[j].toLowerCase();
        ...
        if (allDevicesTagAreGood) {
          ...
          for (let j = 0; j < researchConstraints.utensils.length; j++) {
            let utensilTagToLowerCase =
              researchConstraints.utensils[j].toLowerCase();
